...
class OpenGraph implements Iterator
{
   ...
   */
	 ...
	 * @param unknown $page
	 */
	private static function _fill(&$doc,&$page) {
		$title =  $doc->getElementsByTagName('title');
		$page->_pageMeta['title'] = ($title->length > 0) ? $title->item(0)->textContent : false;

		$randomPara = $doc->getElementsByTagName('p');
		$page->_pageMeta['description'] = ($randomPara->length > 0) ? $randomPara->item(0)->textContent : false;

		$images = $doc->getElementsByTagName('img');
		$page->_pageMeta['image'] = ($images->length > 0) ? $images->item(0)->getAttribute('src') : false;
   ...
   */
	...
	public function getPageMeta($key=false) {
		if($key) {
			return isset($this->_pageMeta[$key]) ? $this->_pageMeta[$key] : false;
		}
		return (object)$this->_pageMeta;
	}
  /**
