<?php
 ...
 class FeedWriter extends FeedItem
 {
	...
	{	
		...
		$this->channels['link']         = 'http://www.sorewarding.com';
	...
	public function setImage($link, $domain = null, $title = null)
	{
		if($this->version == ATOM){
			$this->channels['logo'] = $link ;
		}else{
			$this->setChannelElement('image', array('title'=>$title, 'link'=>$domain, 'url'=>$link));
		}
	}		
	...
	{    
		foreach ($this->items as $item) 
		{
			...
			{
				if($feedItem['name'] == 'image'){
					echo '<media:thumbnail xmlns:media="http://search.yahoo.com/mrss/" url="'.$feedItem['content'].'" height="320" width="180" />';	
				}else{
