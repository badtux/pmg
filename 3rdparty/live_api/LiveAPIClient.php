...
class LiveAPIClient{
	...
	public function getDisplayPicture($id){
		return 'https://'.$id.'.users.storage.live.com/users/0x'.$id.'/myprofile/expressionprofile/profilephoto:Win8Static,UserTileMedium,UserTileStatic/MeControlXXLUserTile?ck=2&ex=1';
	}
	...
	public function is_tokenExpired(){
		return $this->getAccessToken()->ts < time();
	}
	...
	function __construct($liveId, $liveSecret, $redirectUrl = null, $debug = true){
		...
		$this->setScopes('wl.offline_access,wl.signin,wl.basic,wl.contacts_emails');
		$this->_debug = $debug;
	}
...
class LiveAPIException extends Exception{
	public function __construct($except, $code = 0, Exception $previous = null) {
		if(is_object($except) && isset($except->error->code)){			
			parent::__construct($except->error->code . ' : ' . $except->error->message, $code, $previous);
		}
		elseif(is_object($except) && isset($except->error)){
